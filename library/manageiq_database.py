...
def main():
    module = AnsibleModule(
        ...
        required_if=[
            [ "state", "replicate", [ "replication", "cluster_node_number", "username", "password", "dbname", "primary_host"]],
    ...
    if state in ['absent', 'destroy']:
        ...
        cmd += " --replication=%s --primary-host=%s" % (module.params['replication'], module.params['primary_host'])
        if module.params['replication'] == 'standby':
            cmd += " --standby_host=%s --auto-failover" % module.params['standby_host']
